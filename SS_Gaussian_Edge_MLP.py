...
writer = SummaryWriter('runs/SS_Gaussian_Edge_MLP'+str(args.hidden_dim)+'_'+str(args.data)+
...
class SFunc(nn.Module):
    ...
    def __init__(self, hidden_dim1, hidden_dim2, target_dim, temp, gamma_prior, sigma_0):
        ...
        self.l1 = SSGauss_Edge_layer(28*28, hidden_dim1, temp=temp, gamma_prior=gamma_prior,
                                        sigma_0=sigma_0)
...
class RunManager():
    ...
    def end_epoch(self,epoch,train_loss,train_accuracy,test_loss,
                    test_accuracy,sparsity_mlp1,sparsity_mlp2,
...
def main():
    ...
    if args.hidden_dim == 400:
        total_num_para = 478410.
        total_flops = 478410.

    PATH = args.results_path + args.data + '/SS_Gauss_Edge/MLP' +str(args.hidden_dim) + '/'
    print(PATH)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            for _ , (images, labels) in enumerate(test_loader):
                ...
                for it in range(num_MC_test):       
                    ...
                    for _ , module in net.named_modules():
                        if isinstance(module, (SSGauss_Edge_layer)):
            ...
            # flops_pruned_val = (net.l1.input_dim+1)*torch.sum(one1_l) + (torch.sum(one1_l)+1)*torch.sum(one2_l) + (torch.sum(one2_l)+1)*target_dim
            # param_pruned_val = (net.l1.w != 0).sum() + (net.l1.v != 0).sum() + \
        ...
        print('Epoch {}, Train Loss: {}, Train Accuracy: {}, Test Loss: {}, Test Accuracy: {}, \
                Sparsity MLP layer-1: {}, Sparsity MLP layer-2: {} Edge sparsity: {}, \
                FLOPs ratio: {}, Param pruned: {}, FLOPs pruned: {}'.format(
                    epoch, train_loss, train_accuracy, test_loss, test_accuracy, 
                    sparsity_mlp1_val, sparsity_mlp2_val, 
        ...
        m.end_epoch(epoch, train_loss, train_accuracy, test_loss, test_accuracy, 
                    sparsity_mlp1_val, sparsity_mlp2_val,sparsity_overall, param_pruned_val,
                    flops_ratio_val, flops_pruned_val, learning_rate, args.batch_train)

    print('Finished Training')
    writer.close()
    
    m.save(PATH,'results_SS_Gaussian_Edge_MLP'+str(args.hidden_dim)+'_'+str(args.data)+
                                '_relu_sig0_'+str(args.sigma_0)+'_lr_'+str(args.init_lr)+
                                '_batch_'+ str(args.batch_train)+'_total_epochs_'+str(args.nepoch))
    torch.save(net.state_dict(), PATH + 'SS_Gaussian_Edge_MLP'+str(args.hidden_dim)+'_'+str(args.data)+
                                '_relu_sig0_'+str(args.sigma_0)+'_lr_'+str(args.init_lr)+
    ...
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"SS_Gaussian_Edge_MLP400_MNIST_loss_relu_0.001_1024_1200.png"),dpi=300)
    # plt.close()
    ...
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"SS_Gaussian_Edge_MLP400_MNIST_accuracy_relu_0.001_1024_1200.png"),dpi=300)
    # plt.close()
    ...
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"SS_Gaussian_Edge_MLP400_MNIST_sparsity_node_relu_0.001_1024_1200.png"),dpi=300)
    # plt.close()
    ...
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"SS_Gaussian_Edge_MLP400_MNIST_sparsity_edge_relu_0.001_1024_1200.png"),dpi=300)
    # plt.close()
