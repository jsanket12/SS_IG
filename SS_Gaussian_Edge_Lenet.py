...
from Gauss_linear_layers import SSGauss_Edge_layer
from Gauss_Conv_layers import SSGauss_Edge_Conv2d_layer
...
writer = SummaryWriter('runs/SS_Gaussian_Edge_FLOPs_Modified_Lenet_Caffe_'+str(args.data)+
...
class SFunc(nn.Module):
    """
        Our BNN
    """
    def __init__(self, hidden_dim1, hidden_dim2, target_dim, temp, gamma_prior, sigma_0):

        # initialize the network using the MLP layer
        super().__init__()

        # Spike-and-slab Gaussian node selection
        self.conv1 = SSGauss_Edge_Conv2d_layer(1, 20, kernel_size=5, stride=1, temp=temp, 
                                        gamma_prior=gamma_prior, sigma_0=sigma_0)
        self.conv2 = SSGauss_Edge_Conv2d_layer(20, 50, kernel_size=5, stride=1, temp=temp, 
                                        gamma_prior=gamma_prior, sigma_0=sigma_0)
        self.l1 = SSGauss_Edge_layer(4*4*50, hidden_dim1, temp=temp, gamma_prior=gamma_prior,
                                        sigma_0=sigma_0)
        self.l2 = SSGauss_Edge_layer(hidden_dim1, hidden_dim2, temp=temp, gamma_prior=gamma_prior, 
                                        sigma_0=sigma_0)
        self.l3 = SSGauss_Edge_layer(hidden_dim2, target_dim, temp=temp, gamma_prior=gamma_prior,
                                        sigma_0=sigma_0)
    ...
    def forward(self, X):
        ...
        output = F.relu(self.l1(output.reshape(-1,4*4*50)))
...
def main():
    ...
    net = SFunc(hidden_dim1, hidden_dim2, target_dim, temp, gamma_prior, sigma_0).to(device)
    ...
    PATH = args.results_path + args.data + '/SS_Gauss_Edge/'
    print(PATH)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            for _ , (images, labels) in enumerate(test_loader):
                ...
                for it in range(num_MC_test):       
                    ...
                    for _ , module in net.named_modules():
                        if isinstance(module, (SSGauss_Edge_Conv2d_layer)): 
                            # in_prune_channels = module.in_channels
                            if module.transposed:
                                conv_w_in =  module.w_z
                                conv_w_out = module.w_z.permute(1, 0, 2, 3)                                                               
                            else:
                                conv_w_in = module.w_z.permute(1, 0, 2, 3)
                                conv_w_out = module.w_z                                
                            conv_w_in = conv_w_in.reshape(-1, conv_w_in.shape[1]*conv_w_in.shape[2]*conv_w_in.shape[3]).T
                            conv_w_out = conv_w_out.reshape(-1, conv_w_out.shape[1]*conv_w_out.shape[2]*conv_w_out.shape[3]).T
                            if module.v is not None:
                                arr1_l = torch.sum(conv_w_in,0)
                                arr2_l = torch.sum(torch.cat((conv_w_out,module.v_z.expand(1, module.v_z.size()[0])),0),0)
                                in_prune_channels = torch.sum((arr1_l!=0).float())
                                out_prune_channels = torch.sum((arr2_l!=0).float())
                                ...
                                arr1_l = torch.sum(conv_w_in,0)
                                arr2_l = torch.sum(conv_w_out,0)
                                in_prune_channels = torch.sum((arr1_l!=0).float())                                
                                out_prune_channels = torch.sum((arr2_l!=0).float())
                                param_overall += (module.w != 0).sum()
                                flops_overall += out_prune_channels*(in_prune_channels*module.kernel_size[0]*module.kernel_size[1])* \
                                                (np.floor((module.input_size[2]+2*module.padding[0]-module.dilation[0]*(module.kernel_size[0]-1)-1)/module.stride[0]+1))**2/module.groups                        
                        elif isinstance(module, (SSGauss_Edge_layer)):
                            if module.v is not None:
                                arr1_l = torch.sum(torch.cat((module.w_z,module.v_z.expand(1, module.v_z.size()[0])),0),0)
                                arr2_l = torch.sum(torch.cat((module.w_z,module.v_z.expand(1, module.v_z.size()[0])),0),1)
                                output_dim = torch.sum((arr1_l!=0).float())
                                input_dim = torch.sum((arr2_l!=0).float())
                                param_overall += (module.w != 0).sum() + (module.v != 0).sum()
                                flops_overall += input_dim*output_dim
                            else:
                                arr1_l = torch.sum(module.w_z,0)
                                arr2_l = torch.sum(module.w_z,1)
                                output_dim = torch.sum((arr1_l!=0).float())
                                input_dim = torch.sum((arr2_l!=0).float())
                                param_overall += (module.w != 0).sum()
                                flops_overall += input_dim*output_dim
            ...
            sparsity_conv1_val = (torch.sum(one1_l)/(arr1_l.size()[0])).item()

            arr2_l = torch.norm(torch.cat((conv2_w,net.conv2.v.expand(1, net.conv2.v.size()[0])),0),1,0)
            one2_l = (arr2_l!=0).float()
            sparsity_conv2_val = (torch.sum(one2_l)/(arr2_l.size()[0])).item()
    ...
    m.save(PATH,'results_SS_Gaussian_Edge_FLOPs_Modified_Lenet_Caffe_'+str(args.data)+
                                '_relu_sig0_'+str(args.sigma_0)+'_lr_'+str(args.init_lr)+
                                '_batch_'+ str(args.batch_train)+'_total_epochs_'+str(args.nepoch))
    torch.save(net.state_dict(), PATH + 'SS_Gaussian_Edge_FLOPs_Modified_Lenet_Caffe_'+str(args.data)+
                                '_relu_sig0_'+str(args.sigma_0)+'_lr_'+str(args.init_lr)+
    ...
    # plt.title('SS_Gaussian_Edge: Train-Test Loss')
    # plt.xlabel('Epochs')
    # plt.ylabel('Loss')
    # # plt.ylim([0.55, 1.0])
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"SS_Gaussian_Edge_Lenet_Caffe_MNIST_loss_relu_0.001_1024_1200.png"),dpi=300)
    # plt.close()

    # plt.plot(range(num_epochs), train_Accuracy, 'b', label='Train')
    # plt.plot(range(num_epochs), test_Accuracy, 'orange', label='Test')
    # plt.title('SS_Gaussian_Edge: Train-Test Accuracy')
    # plt.xlabel('Epochs')
    # plt.ylabel('Accuracy')
    # # plt.ylim([0.55, 1.0])
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"SS_Gaussian_Edge_Lenet_Caffe_MNIST_accuracy_relu_0.001_1024_1200.png"),dpi=300)
    # plt.close()
    ...
    # plt.title('SS_Gaussian_Edge: Node Sparsity')
    # plt.xlabel('Epochs')
    # plt.ylabel('Sparsity')
    # # plt.ylim([0.65, 1.0])
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"SS_Gaussian_Edge_Lenet_Caffe_MNIST_sparsity_node_relu_0.001_1024_1200.png"),dpi=300)
    # plt.close()

    # plt.plot(range(num_epochs), Edge_sparsity, 'g')
    # plt.title('SS_Gaussian_Edge: Edge Sparsity')
    # plt.xlabel('Epochs')
    # plt.ylabel('Sparsity')
    # # plt.ylim([0.65, 1.0])
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"SS_Gaussian_Edge_Lenet_Caffe_MNIST_sparsity_edge_relu_0.001_1024_1200.png"),dpi=300)
    # plt.close()
