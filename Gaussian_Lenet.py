...
writer = SummaryWriter('runs/Gaussian_Lenet_Caffe_'+str(args.data)+
...
class SFunc(nn.Module):
    ...
    def __init__(self, hidden_dim1, hidden_dim2, target_dim, sigma_0):
        ...
        self.conv1 = Gauss_Conv2d_layer(1, 20, kernel_size=5, stride=1, sigma_0=sigma_0)
        self.conv2 = Gauss_Conv2d_layer(20, 50, kernel_size=5, stride=1, sigma_0=sigma_0)
        self.l1 = Gauss_layer(4*4*50, hidden_dim1, sigma_0=sigma_0)
...
def main():
    ...
    PATH = args.results_path + args.data + '/Gaussian/'
    print(PATH)
        ...
        with torch.no_grad():
            ...
            for _ , (images, labels) in enumerate(test_loader):
                ...
                for it in range(num_MC_test):       
                    ...
                    #     if isinstance(module, (Gauss_Conv2d_layer)): 
                    #         in_prune_channels = module.in_channels                   
                    #         out_prune_channels = module.out_channels
                    #         if module.v is not None:
                    #             param_overall += module.w_mu.numel() + module.v_mu.numel()
                    ...
                    #         else:
                    #             param_overall += module.w_mu.numel() 
                    ...
                    # print(f"Total num para= {param_overall}, and flops is {flops_overall.item()}")
    ...
    m.save(PATH,'results_Gaussian_Lenet_Caffe_'+str(args.data)+
                                '_relu_sig0_'+str(args.sigma_0)+'_lr_'+str(args.init_lr)+
                                '_batch_'+ str(args.batch_train)+'_total_epochs_'+str(args.nepoch))
    torch.save(net.state_dict(), PATH + 'Gaussian_Lenet_Caffe_'+str(args.data)+
                                '_relu_sig0_'+str(args.sigma_0)+'_lr_'+str(args.init_lr)+
    ...
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"Gaussian_Lenet_Caffe_MNIST_loss_relu_0.001_1024_1200.png"),dpi=300)
    # plt.close()
    ...
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"Gaussian_Lenet_Caffe_MNIST_accuracy_relu_0.001_1024_1200.png"),dpi=300)
    # plt.close()
    ...
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"Gaussian_Lenet_Caffe_MNIST_sparsity_node_relu_0.001_1024_1200.png"),dpi=300)
    # plt.close()
    ...
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"Gaussian_Lenet_Caffe_MNIST_sparsity_edge_relu_0.001_1024_1200.png"),dpi=300)
    # plt.close()
