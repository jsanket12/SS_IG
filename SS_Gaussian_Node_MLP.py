...
writer = SummaryWriter('runs/SS_Gaussian_Node_FLOPs_Modified_1_MLP'+str(args.hidden_dim)+'_'+str(args.data)+
...
class SFunc(nn.Module):
    ...
    def __init__(self, hidden_dim1, hidden_dim2, target_dim, temp, gamma_prior, 
        ...
        self.l1 = SSGauss_Node_layer(28*28, hidden_dim1, temp=temp, gamma_prior=gamma_prior_star1, 
                                        sigma_0=sigma_0)
    ...
    def forward(self, X):
        ...
        """        
        output = F.silu(self.l1(X.reshape(-1,28*28)))
...
def main():
    ...
    PATH = args.results_path + args.data + '/SS_Gauss_Node/MLP' +str(args.hidden_dim) + '/'
    print(PATH)
        ...
        with torch.no_grad():
            ...
            for _ , (images, labels) in enumerate(test_loader):
                ...
                for it in range(num_MC_test):       
                    ...
                    #     if isinstance(module, (SSGauss_Node_layer)):
            ...
            flops_pruned_val = ((net.l1.input_dim+1)*torch.sum(one1_l) + \
                                (torch.sum(one1_l)+1)*torch.sum(one2_l) + (torch.sum(one2_l)+1)*target_dim).item()
            param_pruned_val = ((net.l1.w != 0).sum() + (net.l1.v != 0).sum() + \
                                (net.l2.w != 0).sum() + (net.l2.v != 0).sum() + \
                                (net.l3.w != 0).sum() + (net.l3.v != 0).sum()).item()
    ...
    m.save(PATH,'results_SS_Gaussian_Node_FLOPs_Modified_1_MLP'+str(args.hidden_dim)+'_'+str(args.data)+
                                '_silu_sig0_'+str(args.sigma_0)+'_lr_'+str(args.init_lr)+
                                '_batch_'+ str(args.batch_train)+'_total_epochs_'+str(args.nepoch))
    torch.save(net.state_dict(), PATH + 'SS_Gaussian_Node_FLOPs_Modified_1_MLP'+str(args.hidden_dim)+'_'+str(args.data)+
                                '_silu_sig0_'+str(args.sigma_0)+'_lr_'+str(args.init_lr)+
    ...
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"SS_Gaussian_Node_MLP400_MNIST_loss_silu_0.001_1024_1200.png"),dpi=300)
    # plt.close()
    ...
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"SS_Gaussian_Node_MLP400_MNIST_accuracy_silu_0.001_1024_1200.png"),dpi=300)
    # plt.close()
    ...
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"SS_Gaussian_Node_MLP400_MNIST_sparsity_node_silu_0.001_1024_1200.png"),dpi=300)
    # plt.close()
    ...
    # plt.legend(loc='upper right', frameon=False)
    # plt.grid(ls='dotted')
    # plt.savefig(os.path.join(PATH,"SS_Gaussian_Node_MLP400_MNIST_sparsity_edge_silu_0.001_1024_1200.png"),dpi=300)
    # plt.close()
